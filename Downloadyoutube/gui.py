...
class App(ctk.CTk):
    ...
    def __init__(self):
        super().__init__()
        self.selected_transcribe_path = None
        ...
        self.model_var = ctk.StringVar(value=self.get_optimal_model())
        self.selected_transcribe_path = None
    ...
    def select_video_file(self):
        ...
        if file_path:
            self.selected_transcribe_path = file_path
            # Hiển thị tên file lên giao diện cho trực quan
            file_name = os.path.basename(file_path)
            self.file_info_label.configure(text=f"📁 Đã chọn: {file_name}", text_color="#1f538d")
            self.btn_start_transcribe.configure(state="normal", fg_color="#28a745") # Kích hoạt nút trích xuất

    def trigger_transcription(self):
        if self.selected_transcribe_path:
            self.process_transcription(self.selected_transcribe_path)
        else:
            messagebox.showwarning("Chú ý", "Việt chưa chọn file nào mà!")

    def process_transcription(self, video_path):
        ext = os.path.splitext(video_path)[1].lower()       
        try:
            # Vô hiệu hóa nút để tránh nhấn nhiều lần khi đang chạy
            self.btn_start_transcribe.configure(state="disabled")
            ...
            if duration > 300: 
                messagebox.showwarning("Quá dài", "File này dài hơn 5 phút rồi!")
                clip.close()
                self.btn_start_transcribe.configure(state="normal")
                return
                
            self.text_result.delete("0.0", "end")
            self.text_result.insert("0.0", "🤖 Đang khởi động bộ não AI... Vui lòng đợi...\n")
            threading.Thread(target=self.run_whisper, args=(video_path, clip), daemon=True).start()
        except Exception as e:
            messagebox.showerror("Lỗi file", f"Không thể đọc file: {e}")
            self.btn_start_transcribe.configure(state="normal")
    def run_whisper(self, video_path, clip):
        try:
            # 1. Bắt đầu (Dùng đúng biến trans_progress)
            self.trans_progress.set(0.1)
            self.trans_status.configure(text="Đang xử lý âm thanh...", text_color="blue")
            
            audio_path = "temp_audio.mp3"
            
            # KIỂM TRA Ở ĐÂY:
            if hasattr(clip, "audio") and clip.audio is not None:
                # Nếu là VideoClip, ta lấy phần audio của nó
                clip.audio.write_audiofile(audio_path, logger=None)
            else:
                # Nếu là AudioClip, ta dùng chính nó để ghi ra file
    ...
    def init_transcribe_tab(self):
        ctk.CTkLabel(self.tab_transcribe, text="AUDIO TO TEXT PRO", 
                    font=("Segoe UI Variable Display", 24, "bold"), text_color="#1f538d").pack(pady=10)
        
        # --- Khu vực chọn file ---
        self.file_select_frame = ctk.CTkFrame(self.tab_transcribe, fg_color="transparent")
        self.file_select_frame.pack(pady=10)

        self.btn_select_file = ctk.CTkButton(self.file_select_frame, text="📁 1. Chọn Video/Audio", 
                                            height=40, command=self.select_video_file)
        self.btn_select_file.pack(side="left", padx=10)

        # Label hiển thị tên file đã chọn
        self.file_info_label = ctk.CTkLabel(self.tab_transcribe, text="Chưa có file nào được chọn", 
                                            font=("Segoe UI Variable Text", 13, "italic"))
        self.file_info_label.pack(pady=5)

        # --- Khu vực cấu hình và thực hiện ---
        self.action_frame = ctk.CTkFrame(self.tab_transcribe, fg_color="transparent")
        self.action_frame.pack(pady=10)

        # Chọn model
        ctk.CTkLabel(self.action_frame, text="Bộ não AI:", font=("Segoe UI Variable Text", 14)).pack(side="left", padx=5)
        self.model_combo = ctk.CTkComboBox(self.action_frame, 
                                            values=["tiny", "base", "small", "medium", "large"],
                                            variable=self.model_var, state="readonly", width=100)
        self.model_combo.pack(side="left", padx=10)

        # Nút Bắt đầu trích xuất (Mặc định bị tắt cho đến khi chọn file)
        self.btn_start_transcribe = ctk.CTkButton(self.action_frame, text="⚡ 2. Bắt đầu trích xuất", 
                                                height=40, fg_color="gray", state="disabled",
                                                command=self.trigger_transcription)
        self.btn_start_transcribe.pack(side="left", padx=10)

        # Nút Xuất file Text
        self.btn_export_txt = ctk.CTkButton(self.action_frame, text="💾 Xuất .txt", 
        ...
        self.trans_progress.pack(fill="x", padx=40, pady=(15, 0))
        self.trans_progress.set(0)

        self.trans_status = ctk.CTkLabel(self.tab_transcribe, text="Sẵn sàng phục vụ Việt!", 
        ...
        self.text_result.pack(pady=15, padx=30, fill="both", expand=True)